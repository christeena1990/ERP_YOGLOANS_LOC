...
{
    public class Bond_Joint_ApplicantsController : BaseController
    {
        // GET: Bond_Joint_Applicants
       
        DB dbconnect = new DB();
        // GET: NCD_Joint_Applicants

        public ActionResult Bond_Joint_Applicants()
        {
            //Session["application_subid"] = 160;

            var JointaccountDetails = ApplicantsGrid();
            ViewBag.JointaccountDetails = JointaccountDetails;

            // Check if the session variable 'application_custId_160' is not null
            if (Session["application_custId_160"] != null)
            {
        ...
        public DataTable ApplicantsGrid()
        {
            ...
            dbconnect.Open();
            dt = dbconnect.ExecuteDataset("[dbo].[BOND_jointapplication]", pr).Tables[0];
            dbconnect.Close();
            return dt;
        ...
        public DataTable Applicantdetails()
        {
            ...
            pr[1].Value = Session["application_custId_160"]; // Ensure branch_id is converted to an integer
        ...
        public ActionResult JointAplcnt_Btn_clk(string applicationIdTextbox, string jointApplicantname)
        {
            ...
            prr[2].Value = Session["application_custId_160"]; // Assuming you have the customer ID stored in the session
            ...
            dbconnect.ExecuteStoredProcedure("[dbo].[BOND_jointapplication]", prr);
            ...
            Session.Remove("application_custId_160");


            return RedirectToAction("Bond_Joint_Applicants");
        }
