...
{
    public class PL_settlement_Branch_ApproveController : Controller
    {
        DB dbconnect = new DB();

        //PERSONAL LOAN -SETTLEMENT  BRANCH APPROVE 
        //---------------------------------------
        // GET: PL_settlement_Branch_Approve
        public ActionResult PL_Settle_Branch_Approve_View()
        {
            try
            {
                var pl_table_values = PL_Settle_Approve_BranchGrid();
        ...
        private DataTable PL_Settle_Approve_BranchGrid()
        {
        ...
        [HttpPost]
        public JsonResult PL_Branch_settle_details(String Loanno,string slno)
        {
            ...
            //pr1[2] = new SqlParameter("@branch_id", SqlDbType.Int);
            //pr1[2].Value = Session["branch_id"];
        ...
        //public ActionResult Settlement_Approve(String LNo, string outstand, string susp, string settle_int, string f_chg, string o_chg, string discount, string settle_amt, string paymode, string actionType)
        public ActionResult Settlement_Approve( string actionType)

        {
            ...
            try
            {
                if (actionType == "Approve")
                {
                    ...
                    pr[2] = new SqlParameter("@data", SqlDbType.VarChar,100);
                    ...
                    dbconnect.Open();
                    dbconnect.ExecuteStoredProcedure("[dbo].[pl_settlemet_entry]", pr);
                    dbconnect.Close();
                    ...
                    if (messageParts[1] != "")
                    {
                        message = messageParts[1].ToString();
