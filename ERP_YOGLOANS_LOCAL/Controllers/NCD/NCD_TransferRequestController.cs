...
{
    public class NCD_TransferRequestController : Controller
    {
        DB dbconnect = new DB();
        ...
        public ActionResult NCD_TransferRequest()
        {
            return View();
        }
        ...
        [HttpGet]
        public ActionResult GetDetails(string issue_no, long deb_id, string cerno)
        {
            SqlParameter[] pr = new SqlParameter[4];
            pr[0] = new SqlParameter("@query_id", SqlDbType.Int) { Value = 2 };
            pr[1] = new SqlParameter("@deb_id", SqlDbType.BigInt) { Value = deb_id };
            ...
            var ncdTransferRequestDetails = new List<NCD_Transfer_Model>();
            ...
            if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    ncdTransferRequestDetails.Add(new NCD_Transfer_Model()
                    {
                        ...
                        Deb_id = deb_id,  // Assuming you want to keep track of the deb_id
                        Certificate_No = cerno // Assuming this is also relevant to store
                    });
                }
            }

            return Json(ncdTransferRequestDetails, JsonRequestBehavior.AllowGet);
        }
        ...
        [HttpPost]
        public ActionResult SubmitTransferRequest(NCD_Transfer_Model model)
        {
            try
            {
               ...
               // byte[] imageBytes = null;
                ...
                parameters[4] = new SqlParameter("@deb_id", SqlDbType.BigInt) { Value = model.Deb_id }; // Now populated
                ...
                dbconnect.Open();
                dbconnect.ExecuteStoredProcedure("[dbo].[DEB_transfer_save]", parameters);
                dbconnect.Close();
