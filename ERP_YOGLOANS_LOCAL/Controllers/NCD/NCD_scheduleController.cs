...
{
    public class NCD_scheduleController : Controller
    {
        DB dbconnect = new DB();
        // GET: NCD_schedule
        public ActionResult NCD_schedule()
        {
            return View();
        }
        ...
        public JsonResult fill_schemeType()
        {
            ...
            dbconnect.Open();
            dt1 = dbconnect.ExecuteDataset("[dbo].[DEB_report]", pr1).Tables[0];
            dbconnect.Close();
            ...
            try
            {
                ...
                foreach (DataRow row in ds.Tables[0].Rows)
                {
                    var rowData = new
                    {
                        ...
                        Dist_To = row["Dist_To"],
                        Allotment_Dt = row["Allotment_Dt"],
                        Matirity_Dt = row["Matirity_Dt"],
                        Deb_ID = row["Deb_ID"],
