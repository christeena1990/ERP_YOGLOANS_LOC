...
{
    public class NCD_Transfer_JointController : BaseController
    {
        DB dbconnect = new DB();
        // GET: NCD_Transfer_Joint
        public ActionResult NCD_Transfer_Joint()
        {

                //Session["application_subid"] = 152;

                var JointaccountDetails = ApplicantsGrid();
                ViewBag.JointaccountDetails = JointaccountDetails;

                // Check if the session variable 'application_custId_152' is not null
                if (Session["application_custId_152"] != null)
                {
        ...
        public DataTable ApplicantsGrid()
            ...
            public DataTable Applicantdetails()
            {
                ...
                pr[1].Value = Session["application_custId_152"]; // Ensure branch_id is converted to an integer
            ...
            public ActionResult JointAplcnt_Btn_clk(string applicationIdTextbox, string jointApplicantname, string issue_no, string certif, string deb_id)
             {
                ...
                prr[2].Value = Session["application_custId_152"]; // Assuming you have the customer ID stored in the session
                ...
                prr[7] = new SqlParameter("@deb_id", SqlDbType.VarChar, 4000);
                prr[7].Value = deb_id; // Pass the applicant name from the form input

         

            // Execute the stored procedure
            dbconnect.ExecuteStoredProcedure("[dbo].[DEB_transfer_save]", prr);
            ...
            Session.Remove("application_custId_152");

          
            return RedirectToAction("NCD_Transfer_Joint");
